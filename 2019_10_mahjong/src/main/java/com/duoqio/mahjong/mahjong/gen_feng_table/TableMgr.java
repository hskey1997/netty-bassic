package com.duoqio.mahjong.mahjong.gen_feng_table;
...
public class TableMgr {
    ...
    public boolean load() {
        for (int i = 0; i < 9; ++i) {
            String path = "D:/tbl/table_";
            m_check_table[i].load(path + i + ".tbl");
        }

        for (int i = 0; i < 9; ++i) {
            String path = "D:/tbl/eye_table_";
            m_check_eye_table[i].load(path + i + ".tbl");
        }

        for (int i = 0; i < 9; ++i) {
            String path = "D:/tbl/feng_table_";
            m_check_feng_table[i].load(path + i + ".tbl");
        }

        for (int i = 0; i < 9; ++i) {
            String path = "D:/tbl/feng_eye_table_";
